...
/**
 * SystemAdminInvitationRepositoryImpl の統合テスト
 *
 * @author Jun Kobayashi
 */
@DataJpaTest
@AutoConfigureTestDatabase(replace = Replace.NONE)
@Import(SystemAdminInvitationRepositoryImpl.class)
class SystemAdminInvitationRepositoryImplIT extends AbstractRepositoryIT {

    /** テスト対象 */
    @Autowired
    private SystemAdminInvitationRepositoryImpl sut;

    /** テストデータ投入用JPAリポジトリ */
    @Autowired
    private SystemAdminInvitationJpaRepository jpaRepository;
    ...
    void findLatestByInvitationId_returnsLatestVersion_whenMultipleVersionsExist() {
        ...
        saveEntity("inv-1", v1, "hash-1", v1.plusDays(1), "User v1", "creator-1");
        saveEntity("inv-1", v3, "hash-3", v3.plusDays(1), "User v3", "creator-3");
        saveEntity("inv-1", v2, "hash-2", v2.plusDays(1), "User v2", "creator-2");

        Optional<SystemAdminInvitation> result = sut.findLatestByInvitationId("inv-1");
    ...
    void findLatestByInvitationId_returnsSingleVersion_whenOnlyOneExists() {
        saveEntity("inv-only", BASE_TIME, "hash-x", BASE_TIME.plusDays(1),
                "Sole User", "creator");

        Optional<SystemAdminInvitation> result = sut.findLatestByInvitationId("inv-only");
    ...
    void findLatestByInvitationId_returnsEmpty_whenNotFound() {
        Optional<SystemAdminInvitation> result = sut.findLatestByInvitationId("missing");
    ...
    void findLatestByInvitationId_returnsEmpty_whenOtherIdsExist() {
        saveEntity("inv-A", BASE_TIME, "hash-A", BASE_TIME.plusDays(1),
                "A", "creator-A");

        Optional<SystemAdminInvitation> result = sut.findLatestByInvitationId("inv-B");
    ...
    void findByTokenHash_returnsRecord_whenExists() {
        saveEntity("inv-1", BASE_TIME, "unique-hash", BASE_TIME.plusDays(1),
                "User", "creator");

        Optional<SystemAdminInvitation> result = sut.findByTokenHash("unique-hash");
    ...
    void findByTokenHash_returnsOldVersionRecord_whenSearchedWithOldHash() {
        OffsetDateTime v1 = BASE_TIME;
        OffsetDateTime v2 = BASE_TIME.plusHours(1);
        saveEntity("inv-1", v1, "hash-old", v1.plusDays(1), "User v1", "creator");
        saveEntity("inv-1", v2, "hash-new", v2.plusDays(1), "User v2", "creator");

        // 旧バージョンのhashで検索 → 該当する旧バージョンレコードが返る
        Optional<SystemAdminInvitation> result = sut.findByTokenHash("hash-old");
    ...
    void findByTokenHash_returnsEmpty_whenNotFound() {
        saveEntity("inv-1", BASE_TIME, "exists", BASE_TIME.plusDays(1), "User", "creator");

        Optional<SystemAdminInvitation> result = sut.findByTokenHash("nonexistent");
    ...
    void findByTokenHash_returnsEmpty_whenTableIsEmpty() {
        Optional<SystemAdminInvitation> result = sut.findByTokenHash("any");

        assertThat(result).isEmpty();
    }

    @Test
    @DisplayName("findAllLatest: 複数のinvitation_idがある場合、各IDの最新versionのみを返す")
    void findAllLatest_returnsLatestVersionPerInvitationId() {
        OffsetDateTime v1 = BASE_TIME;
        OffsetDateTime v2 = BASE_TIME.plusHours(1);
        saveEntity("inv-A", v1, "A-v1", v1.plusDays(1), "A v1", "creator");
        saveEntity("inv-A", v2, "A-v2", v2.plusDays(1), "A v2", "creator");
        saveEntity("inv-B", v1, "B-v1", v1.plusDays(1), "B v1", "creator");

        List<SystemAdminInvitation> result = sut.findAllLatest();

        assertThat(result).hasSize(2);
        assertThat(result)
                .extracting(SystemAdminInvitation::getInvitationId,
                        SystemAdminInvitation::getTokenHash)
                .containsExactlyInAnyOrder(
                        org.assertj.core.api.Assertions.tuple("inv-A", "A-v2"),
                        org.assertj.core.api.Assertions.tuple("inv-B", "B-v1"));
    }

    @Test
    @DisplayName("findAllLatest: テーブルが空の場合、空のリストを返す")
    void findAllLatest_returnsEmpty_whenTableIsEmpty() {
        List<SystemAdminInvitation> result = sut.findAllLatest();
    ...
    @Test
    @DisplayName("findAllLatest: 1件のみの場合、その1件を返す")
    void findAllLatest_returnsSingle_whenOnlyOneExists() {
        saveEntity("inv-1", BASE_TIME, "hash-1", BASE_TIME.plusDays(1),
                "User", "creator");
    ...
    void save_persistsNewRecord_withAllFields() {
        ...
        SystemAdminInvitation domain = new SystemAdminInvitation(
                "inv-new", version, "hash-new", expiresAt,
                "Display Name", version, "creator-new");

        sut.save(domain);

        Optional<SystemAdminInvitation> result = sut.findLatestByInvitationId("inv-new");
        assertThat(result).isPresent();
        SystemAdminInvitation persisted = result.get();
    ...
    void save_savesMultipleVersionsForSameInvitationId() {
        ...
        sut.save(new SystemAdminInvitation(
                "inv-1", v1, "hash-v1", v1.plusDays(1), "User", v1, "creator"));
        sut.save(new SystemAdminInvitation(
                "inv-1", v2, "hash-v2", v2.plusDays(1), "User", v2, "creator"));

        assertThat(jpaRepository.count()).isEqualTo(2);
        Optional<SystemAdminInvitation> latest = sut.findLatestByInvitationId("inv-1");
    ...
    private void saveEntity(
            ...
            String displayName,
            String createdBy) {
        SystemAdminInvitationEntity entity = new SystemAdminInvitationEntity(
                new SystemAdminInvitationId(invitationId, version),
                tokenHash,
                expiresAt,
                displayName,
                version,
